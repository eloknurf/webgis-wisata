...
<script src="https://unpkg.com/leaflet@1.3.4/dist/leaflet.js" integrity="sha512-nMMmRyTVoLYqjP9hrbed9S+FzjZHW5gY1TWCHA5ckwXZBadntCNs8kEqAWdrb9O7rxbCaA4lKTIWjDXZxflOcA=="
 ...
 <script src="assets/js/leaflet-routing-machine/examples/Control.Geocoder.js"></script>
   ...
   <script type="text/javascript">
   	...
   	var map = L.map('mapid').setView([-7.656213997067591, 112.6004898679028], 11);

    var LayerKita=L.tileLayer('http://{s}.tile.osm.org/{z}/{x}/{y}.png', {
	attribution: '&copy; <a href="http://osm.org/copyright">OpenStreetMap</a> contributors',
	});
	map.addLayer(LayerKita);
    L.marker([-7.656213997067591, 112.6004898679028]).addTo(map);
...
// marker
	...
	foreach ($getdata as $row) {
		...
		L.marker([<?=$row->lat?>,<?=$row->lng?>],{icon: myIcon}).addTo(map)
				.bindPopup("Nama Wisata : <?=$row->nm_hotspot?><br>"+
				"Lokasi : <?=$row->lokasi?>,Kec. <?=$row->nm_kecamatan?><br>"+
	...
	var control = L.Routing.control({
	    waypoints: [
	        L.latLng(-7.656213997067591, 112.6004898679028),
	        L.latLng(-7.356213997067591, 112.3004898679028)
	    ],
