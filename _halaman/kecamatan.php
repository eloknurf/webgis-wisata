...
if(isset($_GET['hapus'])){
  ...
  $info='<div class="alert alert-success alert-dismissible">
            <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
            <h4><i class="icon fa fa-ban"></i> Success!</h4> Delete Data Successful </div>';
            if($exec){
              $session->set('info',$info);
            }
            else{
                $session->set("info",'<div class="alert alert-danger alert-dismissible">
                          <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                          <h4><i class="icon fa fa-ban"></i> Error!</h4> Process Failed
                        </div>');
            }
            redirect(url($url));
}
    ...
    <?php
      ...
      foreach ($getdata as $row) {
        ?>
          <tr>
            <th><?=$no?></th>
            <th><?=$row->kd_kecamatan?></th>
            <th><?=$row->nm_kecamatan?></th>
            ...
            <th>
              ...
              <a href="<?=url($url.'&hapus&id='.$row->id_kecamatan)?>" class="btn btn-danger" onclick="return confirm('Delete item?')"><i class="fa fa-trash"></i> Hapus</a>
            </th>
          </tr>
        <?php
        $no++;
